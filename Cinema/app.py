...
@app.route('/editar_filme/<string:titulo>', methods=['GET', 'POST'])
def editar_filme(titulo):
    # Busca o filme pelo título exato (case-sensitive)
    filme = Filme.query.filter_by(titulo=titulo).first_or_404()
...
def ListarD():
    ...
    return render_template('ListaD.html', diretores = diretores)

@app.route('/editar')
def editar():
    Filmes = Filme.query.all()

    return render_template('Editar.html', filmes = Filmes)
...
@app.route('/adicionar_diretor', methods=["GET", "POST"])
def adicionar_diretor():
    if request.method == 'POST':
        nome = request.form['nome']
        nacionalidade = request.form['nacionalidade']
        filmes_dirigidos = request.form['filmes_dirigidos']
        ...
        return redirect(url_for('listar_diretores'))
    
    return render_template('DiretoresCrud.html', diretor=None)

@app.route('/listar_diretores')
def listar_diretores():
    diretores = Diretor.query.all()

    return render_template('DiretoresLista.html', diretores = diretores)

@app.route('/editar_diretor/<int:id>', methods=['GET', 'POST'])
def editar_diretor(id):
    diretor = Diretor.query.get_or_404(id)
    
    if request.method == 'POST':
        diretor.Nome = request.form['nome']
        diretor.Nacionalidade = request.form['nacionalidade']
        diretor.Filmes_dirigidos = request.form['filmes_dirigidos']
        diretor.Data_nascimento = request.form['data_nascimento']
        
        db.session.commit()
        flash('Diretor editado com sucesso!', 'success')
        return redirect(url_for('listar_diretores'))
    
    return render_template('editar_diretor.html', diretor=diretor)

@app.route('/deletar_diretor/<int:id>', methods=['GET', 'POST', 'DELETE'])
def deletar_diretor(id):
    diretor = Diretor.query.get(id)
    if diretor:
        db.session.delete(diretor)
        db.session.commit()
        flash('Diretor deletado com sucesso!', 'success')
    else:
        flash('Diretor não encontrado.', 'danger')
